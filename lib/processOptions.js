...
module.exports = (options, req, res, next) => {
  ...
  if (!authorizerApiKey) {
    return error(
      res,
      'express-jwt-aserto: must provide authorizerApiKey in option map'
    );
  }

  // set the tenant ID
  const tenantId =
    options && typeof options.tenantId === 'string' && options.tenantId;
  if (!tenantId) {
    return error(
      res,
      'express-jwt-aserto: must provide tenantId in option map'
    );
  }
