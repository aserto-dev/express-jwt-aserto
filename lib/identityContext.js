  ...
  if (useAuthorizationHeader) {
    try {
      ...
      const token = req.headers.authorization
        ...
        : req.oidc.idToken
        ? `Bearer ${req.oidc.idToken}`
        : null;
      const decodedToken = jwt_decode(token);
      identityContext.identity = decodedToken && decodedToken.sub;
      ...
      identityContext.identity = token.replace('Bearer ', '');
      identityContext.type = 'IDENTITY_TYPE_JWT';
    } catch (error) {
      log(
        `express-jwt-aserto: Authorization header contained malformed JWT: ${error.message}`,
        'ERROR'
      );
