...
module.exports = (policy, resourceKey, optionsParam) => {
  ...
  if (typeof resourceKey !== 'string') {
    throw new Error(
      `express-jwt-aserto: parameter "resourceKey" must be a key into the request parameters map, or ''`
    );
  }
  ...
  return async (req, res, next) => {
    ...
    // initialize the resource value from the parameters map based on the resource key
    const resource = resourceKey ? req.params[resourceKey] : '';
